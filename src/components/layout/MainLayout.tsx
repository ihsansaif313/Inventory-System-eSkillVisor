...
const MainLayout = ({
  ...
  return <div className="flex h-screen bg-neutral-light overflow-hidden">
      {/* Mobile sidebar backdrop */}
      {sidebarOpen && <div className="fixed inset-0 z-20 bg-black bg-opacity-50 transition-opacity md:hidden" onClick={() => setSidebarOpen(false)}></div>}
      {/* Sidebar */}
      <div className={`fixed inset-y-0 left-0 z-30 w-64 transform bg-secondary transition duration-300 ease-in-out md:relative md:translate-x-0 ${sidebarOpen ? 'translate-x-0' : '-translate-x-full'}`}>
        <div className="flex h-full flex-col">
            ...
            <div className="flex items-center">
              ...
              <div className="ml-3">
                <p className="text-sm font-medium text-white">
                  {userRole === 'superadmin' ? 'Super Admin' : userRole === 'manager' ? 'Manager' : 'Partner'}
                </p>
          ...
          <div className="border-t border-secondary-dark p-4">
            <button onClick={handleLogout} className="flex w-full items-center px-4 py-3 text-sm text-white text-opacity-80 rounded-lg hover:bg-secondary-dark">
